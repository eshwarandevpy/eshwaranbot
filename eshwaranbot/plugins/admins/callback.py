...
from eshwaranbot import YouTube, app
from eshwaranbot.core.call import Dusky
from eshwaranbot.misc import SUDOERS, db
from eshwaranbot.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from eshwaranbot.utils.decorators.language import languageCB
from eshwaranbot.utils.formatters import seconds_to_min
from eshwaranbot.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from eshwaranbot.utils.stream.autoclear import auto_clean
from eshwaranbot.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await eshwaranbot.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await eshwaranbot.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await eshwaranbot.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await eshwaranbot.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await eshwaranbot.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await eshwaranbot.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
